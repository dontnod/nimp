...
def p4_add(cl_number, path, pattern = '*'):
    ...
    for file in files:
        ...
        if _p4_run_command(".", ["p4", "-z", "tag", "add", "-c", cl_number, file]) is None:
            return False
...
#-------------------------------------------------------------------------------
def p4_get_disk_path(workspace_path):
    result = {}

    return _p4_parse_command_output(".", ["p4", "-z", "tag", "where", workspace_path], "\.\.\. path (.*)")
...
def p4_submit(cl_number):
    ...
    p4_command = ["p4", "-z", "tag", "submit", "-f", "revertunchanged", "-c", cl_number]
