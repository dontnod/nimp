...
def extract(archive_file_name, destination_directory, archive_type = None, before_decompress_file_callback = None):
    log_verbose("[nimp] Determining type of archive {0}", archive_file_name)
    if(archive_type is None):
        ...
        if(first_extension == ".zip"):
            ...
            log_verbose("[nimp] Archive must be a zip")
        elif(first_extension == ".tar"):
            log_verbose("[nimp] Archive must be an uncompressed tarfile")
            archive_type = TAR_FILE
        elif( ( first_extension == ".gz" or first_extension == ".bz2") and ( second_extension == ".tar" ) ):
            log_verbose("[nimp] Archive must be a compressed tarfile")
            archive_type = TAR_FILE
        else:
            log_error("[nimp] Unable to determine archive type of {0} based on it's name. Please specify it", archive_file_name)
            return False
    if(archive_type == TAR_FILE):
        ...
        log_error("[nimp] Bad archive type. Please specify TAR_FILE or ZIP_FILE")

    return False

#-------------------------------------------------------------------------------
def _decompress_tar(file_name, destination_directory, before_decompress_file_callback = None):
    log_verbose("[nimp] Decompressing {0} in {1}", file_name, destination_directory)
    try:
        ...
        log_error("[nimp] Error while opening tarfile {0}: {1}", file_name, tar_error)
        return False
    except IOError as io_error:
        log_error("[nimp] Unable to open tarfile {0}: {1}", file_name, io_error)
        return False

    log_verbose("[nimp] Decompression succeed")
    return True
...
def _decompress_zip(file_name, destination_directory, before_decompress_file_callback = None):
    ...
    try:
        ...
        log_error("[nimp] Error while opening zip {0}: {1}", file_name, zip_error)
        return False
    except IOError as io_error:
        log_error("[nimp] Unable to open zip {0}: {1}", file_name, io_error)
        return False
